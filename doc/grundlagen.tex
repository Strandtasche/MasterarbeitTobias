...
\begin{equation} \label{eqn:MSE-L2}
	J(X, Y) = \frac{1}{m} \sum_{i=1}^{m} (\vec{y}^{(i)} - \hat{\vec{y}}^{(i)})^2 + \sum_{j} \sum_{k} (\mat{W}_{j,k}^2)
\end{equation} 
...
\begin{figure}
	...
	\caption{Schematische Darstellung nach \cite{Pfaff2017} des optischen Bandsortierers TableSort.}
\end{figure}
