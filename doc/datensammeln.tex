...
Was die Reihenfolge angeht, so ist sie für das neuronale Netz irrelevant, 
...
Diese findet man als die nächste Zeile in der Doppelspalte des dazugehörigen Tracks.
...
Jeder Label-Bestandteil \(e\), sei es das erste oder das zweite Element seines entsprechenden Labels, wird gemäß seiner Zugehörigkeit vor dem Training angepasst

\begin{equation*}
	e_{\text{neu}} = \frac{e - \mu_{\text{N}}}{\sigma_{\text{N}}}
\end{equation*}

Die Ausgaben des Netzes müssen umkehrt zurück in ihre ursprüngliche Form gerechnet werden.
...
\todo{Optionnal: Bayer-Matrix erklären}
...
Die Open Source Computer Vision Library OpenCV \footnote{https://opencv.org/} hat eine Methode implementiert, die ein Bild von einem Farbraum in einen anderen übertragen kann.
...
Die Mittelpunkte in dieser CSV-Datei werden nun mittels des in \textsc{Matlab} implementierten \textit{Predictive Tracking} Algorithmus einzelnen Tracks zugeordnet,
...
Als Data Augmentation bezeichnet man Verfahren, die die eigenen Daten erweitert ohne zusätzliche Daten aufzunehmen.
...
Tracks, die eine gewisse Distanz zur Mittelinie überschreiten werden ausgenommen, da zumindest bei den selbst aufgenommenen Daten, 
...
und außerdem ausreichend groß ist um statistisch aussagekräftige Ergebnisse zu produzieren.

Im Rahmen dieser Arbeit wurde meist ein 90:10 Split zwischen Trainings- und Testset verwendet.
